  ...
  for (var i = 0; i < modules.length; i++) {
    ...
    for (var definition in definitions) {
      if(definitions[definition] > module.programInstructions.length - 1) {
        ...
        console.log("Error: Definition of " + definition + " exceeds the size of module " + (i+1))
      }
  ...
  for (var i = 0; i < ms.length; i++) {
    for (var use in ms[i].usageMap) {
      ...
      while (indexOfNextUse != 777) {
        ...
        if(ms[i]["programInstructions"].length > indexOfNextUse) {
          var tmp = ms[i].programInstructions[indexOfNextUse].address;
          ms[i].programInstructions[indexOfNextUse].address = useValue;
          indexOfNextUse = tmp;
        } else {
          console.log("Error: Pointer in use chain exceeds module size; chain terminated.");
          indexOfNextUse = 777;
        }
      }
    }
